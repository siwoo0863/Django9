...
class Post(models.Model):
    ...
    class Meta: #모델 클래스도 Meta 클래스를 정의해서 추가적인 정렬, 정보를 제공
        ...
        #기본적으로 오름차순으로 정렬함. 변수명앞에'_' 기호를 붙으면 내림차순으로 정렬
        ordering = ['-id']
        #관리자 사이트에서 해당 모데클래스 이름을 변경
        verbose_name = '글'
...
class PostImage(models.Model):
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        #해당 객체를 수동으로 지우는 작업(delete함수 호출)을 수행해야 함
        self.image.delete()
