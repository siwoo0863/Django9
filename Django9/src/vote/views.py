...
# 매개변수가 추가로 사용되는 뷰함수 인경우 args 변수에 튜프령식으로 인자값을 저장
# reverse('vote:index')
...
def index(request):
    if request.method=="GET":
        ...
        obj = Comment()#Comment 모델클래스의 객체 생성. 데이터베이스에 반영X
        obj.comment = comment1
        #request.user : 해당 요청을 한 크라이언트의 유저 정보
        obj.user = request.user
        obj.save()
        return HttpResponseRedirect(reverse('vote:index'))
# 질문에 대한 설문지 제공
def detail(request, question_id):
    # get_object_or_404 : 모델클래스의 객체 한 개를 추출, 조건에 맞는
    # 객체가 없는 경우 크라이언트에게 404 에러메시지를 전달
...
#모델클래스의 객체에 저장된 변수값을 가져올때 F객체를 생성해 사용하면
...
def vote(request):
    ...
    if request.method == "POST":
        ...
        # 사용자가 보낸 selcet 값으로 Choice 객체를 찾음
        ...
        return HttpResponseRedirect(
            reverse('vote:result', args=(obj.question.id ,)))
...
def questionUpdate(request, question_id):
    ...
    if request.method == "GET":
        ...
        form = QuestionForm(request.POST, intance=obj)
...
def questionDelete(request, question_id):
    ...
    # 모델클래스의 객체.delete() : 데이터베이스에 해당 객체가 삭제됨
    question = get_object_or_404(Question, id=question_id)
    question.delete()
    return HttpResponseRedirect(reverse('vote:index'))
...
def choiceRegister(request):
    ...
    #      (생성한 폼객체를 템플릿엔진에게 전달)
        return render(request, 'vote/choiceRegister.html',
                      {'form':obj, 'name':'답변 등록 페이지', 'submit':"답변등록"})
    # 1-2)POST
    elif request.method == "POST":
    ...
    # 3)URL 등록 + 링크 생성
# 답변 수정
def choiceUpdate(request, cid):  # 1)매개변수 추가
    # 2)Choice 객체 찾기
    obj = get_object_or_404(Choice, id=cid)
    # 3)GET,POST 분리
    if request.method == "GET":
    # 3-1)GET : ChoiceForm 생성자의 인자로 Choice개체를 입력
    #          HTML 파일 전달
        form = ChoiceForm(instance=obj)
        return render(request, 'vote/choiceRegister.html',
                      {'form':form, 'name' :'답변 수정 페이지', 'submit':'답변수정'})
    elif request.method == "POST":
    # 3-2): ChoiceForm 생성자의 인자로 입력데이터와 Choice객체를 ㅣㅂ력
    #          수정사항을 데이터베이스에 저장
    #          index or detail URL을 클라이언트에게 전송
        ...
        return HttpResponseRedirect(
            reverse('vote:detail', args=(obj.question.id,)))
