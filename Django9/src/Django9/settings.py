...
#LOGIN_URL : login_required 데코레이터/ LoginRequiredMixin 클래스를 통해 보여질 로그인페이지 주소
...
#구글에서 발급받은 클라이언트 ID
SOCIAL_AUTH_GOOGLE_OAUTH2_KEY='408452548578-fe8je1hpj8o3gd94qq9n279j07rudrnr.apps.googleusercontent.com'
#구글에서 발급받은 보안 비밀
SOCIAL_AUTH_GOOGLE_OAUTH2_SECRET='mXu7w-TKwpNOClWet6zKxWDZ'
...
SECRET_KEY = 'st27162-xo=f)dinqunc_ciny6+!yzg#_q27p^_7mn46_ya(!t'
...
#http://127.0.0.1:8000/files/ 로 시작하는 경로는 미디어파일을 불러오는 것으로 판단한
MEDIA_URL ='/files/'
#os.path.join(기존경로, 새경로) : 기존경로에 새경로를 붙인 문자열을 반환
#실제로 이미지/파일/동영상/한글/파워포인트의 저장되는 경로
