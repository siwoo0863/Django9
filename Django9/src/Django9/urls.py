...
#웹서버 실행시 나오는 도메인 주소(127.0.0.1:8000)은 기본값 부여
urlpatterns = [
    #http://127.0.0.1:8000/admin 로 접근시 관리자 사이트 호출
    ...
    #http://127.0.0.1:8000/1/숫자데이터/ 로 접근시 bookdetail함수 호출
    #이 때, 숫자데이터에 값은 bookmark_id 매개변수에 들어감
    path('<int:bookmark_id>/', bookdetail),
    #주소에 http://127.0.0.1:8000/vote 가 포함이 되어있으면, vote폴더안에
    #있는 urls.py에 등록된 url을 사용
    path('vote/', include("vote.urls")),
    #주소에 http://127.0.0.1:8000/auth/ 가 적힌 URL들은
    #customlogin폴더에 있는 urls파일이 관리
